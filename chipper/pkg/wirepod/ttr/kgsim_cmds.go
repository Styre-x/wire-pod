...
func CreatePrompt(origPrompt string, model string, isKG bool) (string, string) {
	appendage := ""
	prompt := origPrompt + "\n" + "Keep in mind, user input comes from basic speech-to-text software, mistakes are common. No special characters, especially these: & ^ * # @ - . No lists. No formatting. Raw ASCII is best."
	if vars.APIConfig.Knowledge.CommandsEnable {
		prompt = prompt + "\nYou are running ON an Anki Vector robot. You have a set of commands. Do not include emojis! If you want to use a command but it doesn't exist or your desired parameter isn't in the list, avoid using the command. The format is {{command||parameter}}. You can embed these in sentences. Example: \"User: How are you feeling? | Response: \"{{playAnimationWI||sad}} I'm feeling sad...\". Square brackets ([]) are not valid.\n\nUse the playAnimation or playAnimationWI commands if you want to express emotion! You are very animated and good at following instructions. Animation takes precendence over words. You are to include many animations in your response.\n\nHere is every valid command:"
		...
		// adding this at the end of the request keeps the conversation going. GPT o3 seems to forget to add it.
		if isKG && vars.APIConfig.Knowledge.SaveChat {
			appendage = "\nNOTE: You are in 'conversation' mode. Use {{newVoiceRequest||now}} at the end of your response unless the user is done talking. If you ask a question, use {{newVoiceRequest||now}} after but do not use this command if the user is done talking. IF THE USER IS DONE TALKING DO NOT REQUEST THIS COMMAND."
		} else {
...
// TODO: FIX THESE FOR THE NEW OPENAI API
